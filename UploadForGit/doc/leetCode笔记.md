...
5.桶排序。给定一个无序数组，找出数组在排序之后，相邻元素之间最大的差值。数组个数大于2。
	...
	分析：
		...
		（2）前提2：如果一个数组为等差递增数组，那么最大间距t=(max-min)/(n-1)。任意向左向右移动其中某个元素，t一定会增加。所以t=(max-min)/(n-1)是满足相同间距、元素数量的数组的[相邻元素之间最大差值]最小值。
...
2.递归
	[0,5]->[0,2]->[0,1]左边到[0,0]，[1,1]两个节点，[0,0]节点的区间不在查询区间内，返回无穷大；[1,1]节点区间为查询区间子区间，返回最小值9。
